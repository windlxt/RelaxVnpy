...
class BaseMonitor(QtWidgets.QTableWidget):
    ...
    signal: QtCore.Signal = QtCore.Signal(engine.Event)

    def __init__(self, main_engine, event_engine) -> None:
        """"""
        super().__init__()

        self.main_engine: engine.MainEngine = main_engine
        self.event_engine: engine.EventEngine = event_engine
...
class ConnectDialog(QtWidgets.QDialog):
    ...
    def __init__(self, main_engine, gateway_name: str) -> None:
        """"""
        super().__init__()
    ...
    def init_ui(self) -> None:
        ...
        # 对于CTP接口，其值为{'用户名': '', '密码': '', ...}
        # 该字典来自CtpGateway类的类属性default_setting
        default_setting: dict = self.main_engine.get_default_setting(
            self.gateway_name)

        # Saved setting provides field data used last time.
        # 取上次连接时保存下来的连接配置信息
        # 对于CTP接口，这些配置信息保存在文件connect_ctp.json中
        loaded_setting: dict = utility.load_json(self.filename)
    ...
    def connect(self) -> None:
        ...
        # 写接口连接配置信息文件
        utility.save_json(self.filename, setting)

        # 根据连接配置信息，调用主引擎的connect方法，连接接口
...
class TradingWidget(QtWidgets.QWidget):
    ...
    signal_tick: QtCore.Signal = QtCore.Signal(engine.Event)
    ...
    def process_tick_event(self, event: engine.Event) -> None:
        """"""
    ...
    def set_vt_symbol(self) -> None:
        ...
        if not contract:
            ...
            # Update price digits
            # 计算价格显示精度
            self.price_digits = utility.get_digits(contract.pricetick)
        ...
        # Subscribe tick data
        # 创建行情订阅请求, SubscribeRequest 在object.py中定义
...
class ContractManager(QtWidgets.QWidget):
    ...
    其“查询”按钮的处理函数show_contracts，并没有向服务器请求查询，而是直接在主引擎中取
    """
    ...
    def show_contracts(self) -> None:
        ...
        # 从主引擎中取所有合约.调用的其实就是OMS引擎的get_all_contracts方法。
...
class GlobalDialog(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        settings: dict = copy(setting.SETTINGS)
        settings.update(utility.load_json(setting.SETTING_FILENAME))
    ...
    def update_setting(self) -> None:
        ...
        utility.save_json(setting.SETTING_FILENAME, settings)
        self.accept()
